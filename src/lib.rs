...
impl LitePool {
    pub async fn new(db: SqlitePool, tables: Tables) -> Result<LitePool> {
        tables.check()?;
    ...
    pub async fn open(dbpath: &str, tables: Tables) -> Result<LitePool> {
        let opts = dbpath
            .parse::<SqliteConnectOptions>().expect("error in dbpath parse")
        ...
        let db = sqlx::sqlite::SqlitePoolOptions::new()
            // .max_connections(1)
            .connect_with(opts)
            .await.expect("error in connect_with");
    ...
    pub async fn init(&self) -> Result<()> {
        sqlx::migrate!("./migrations")
            .run(&self.db)
            .await
            .map_err(|e| format_err!("run sqlite migrations failed: {}", e)).unwrap();

        Ok(())
    }

    #[inline]
    pub fn definition_identity<'a>(&self) -> &'static str {
        self.tables.identity
    }

    #[inline]
    pub fn definition_ratchet_key<'a>(&self) -> &'static str {
        self.tables.ratchet_key
    }

    #[inline]
    pub fn definition_session<'a>(&self) -> &'static str {
        self.tables.session
    }

    #[inline]
    pub fn definition_signed_key<'a>(&self) -> &'static str {
        self.tables.signed_key
    }

    #[inline]
    pub fn definition_pre_key<'a>(&self) -> &'static str {
        self.tables.pre_key
    }
}
...
pub struct Tables {
    ...
    pub ratchet_key: &'static str,
    pub session: &'static str,
    pub signed_key: &'static str,
    pub pre_key: &'static str,
}
...
impl Tables {
    pub fn check(&self) -> Result<()> {
        let strs = [self.identity, self.ratchet_key, self.session, self.signed_key, self.pre_key];
        let mut names = strs.iter().filter(|s| !s.is_empty()).collect::<Vec<_>>();
        if names.len() != strs.len() {
            SignalProtocolError::InvalidArgument(format!("empty table name"));
        }

        names.dedup();
        if names.len() != strs.len() {
            SignalProtocolError::InvalidArgument(format!("duplicate table name"));
        }
...
impl KeyChatIdentityKeyStore{
    ...
    pub async fn create_identity(&self, address: &ProtocolAddress, id_key_pair: &IdentityKeyPair) -> Result<bool> {
        ...
        let identity = self.get_identity_by_address(name, &device_id.to_string()).await?;
        if identity.is_none() {
    ...
    pub async fn delete_identity(&self, address: &str) -> Result<bool> {
        let sql = format!("delete from {} where address = ?", self.pool.definition_identity());
...
impl IdentityKeyStore for KeyChatIdentityKeyStore {
    ...
    async fn save_identity(
        ...
        let name = address.name();
        let device_id = address.device_id();
        let mut signal_identity = self.get_identity_by_address(name, &device_id.to_string()).await?;
        // new key
        ...
        // overwrite
        if self.get_identity_public_key(&signal_identity.as_ref().unwrap().public_key)? != *identity {
            signal_identity.as_mut().unwrap().public_key = format!("{:?}", identity.serialize());
            self.insert_identity(signal_identity.unwrap()).await?;
            return Ok(true);
        }
        // same key
        Ok(false)
    ...
    async fn is_trusted_identity(
        ...
        let signal_identity = self.get_identity_by_address(their_address, &device_id).await?;
        match direction {
            Direction::Sending => {
                if signal_identity.is_none() {
                    return Ok(true);
                }
                if *identity != self.get_identity_public_key(&signal_identity.as_ref().unwrap().public_key)? {
                    return Ok(false);
                }
                return Ok(true)
            }
    ...
    async fn get_identity(&self, address: &ProtocolAddress) -> Result<Option<IdentityKey>> {
        ...
        let id_key = self.get_identity_public_key(&identity.unwrap().public_key)?;
        Ok(Some(id_key))
    }
}
...
impl KeyChatSessionStore {
    ...
    pub async fn insert_session(&self, address: &ProtocolAddress, record: &SessionRecord, 
        ...
        if my_receiver_address.is_none() && to_receiver_address.is_none() {
            let sql = format!("INSERT INTO {} (address, device, record) values (?, ?, ?)", self.pool.definition_session());
            sqlx::query(&sql)
                .bind(&address.name())
                .bind(&address.device_id().to_string())
                .bind(format!("{:?}", record.serialize().unwrap()))
                .execute(&self.pool.db)
                .await
    ...
    pub async fn get_all_alice_addrs(&self) -> Result<Vec<String>> {
        ...
        let sql = format!("select aliceAddresses from {}", self.pool.definition_session());
        ...
        while let Some(it) = iter.next().await {
            ...
            if address.is_some() {
                // let address2 = address.unwrap();
                // if address2.contains(",") {
                //     let list_addr = address2.split(",");
                //     for addr in list_addr {
                //         alice_addrs.push(addr.to_string());
                //     }
                // } else {
                //     alice_addrs.push(address2);
                // }
                alice_addrs.push(address.unwrap());
            }
        }
        Ok(alice_addrs)
    }
    ...
    pub async fn get_alice_addrs_by_identity(&self, address: &str, device_id: &str) -> Result<Vec<String>> {
        ...
        // aliceAddresses is binary list combine
        if address.is_some() {
            // let address2 = address.unwrap();
            // if address2.contains(",") {
            //     let list_addr = address2.split(",");
    ...
    pub async fn update_alice_addr(&self, address: &str, device_id: &str, alice_addr: &str) -> Result<bool> {
        ...
        let sql = format!("update {} set aliceAddresses = ? where address = ? and device = ? ", self.pool.definition_session());
        let result = sqlx::query(&sql)
            .bind(alice_addr)
            .bind(address)
            .bind(device_id)
    ...
    pub async fn delete_session(&self, address: &ProtocolAddress) -> Result<()> {
        ...
        if session.is_some() {
            let sql = format!("delete from {} where address = ? and device = ?", self.pool.definition_session());
            sqlx::query(&sql)
                .bind(name)
                .bind(device_id)
                .execute(&self.pool.db)
    ...
    pub async fn delete_session_by_device_id(&self, device_id: u32) -> Result<bool> {
        let sql =  format!("delete from {} where device = ?", self.pool.definition_session());
        let result = sqlx::query(&sql)
            .bind(device_id)
            .execute(&self.pool.db)
            .await
    ...
    pub async fn contains_session(&self, address: &ProtocolAddress) -> Result<bool> {
        ...
        let flag = session_record.clone().unwrap().has_usable_sender_chain(SystemTime::now()).unwrap() 
...
impl KeyChatRatchetKeyStore {
    ...
    pub async fn insert_ratchet_key(&self, ratchet_key: SignalRatchetKey) -> Result<()>{
     ...
     pub async fn delete_by_ratchet_key(&self, ratchet_key: &str) -> Result<()>{
        ...
        let sql = format!("delete from {} where aliceRatchetKeyPublic = ?", self.pool.definition_ratchet_key());
        let result = sqlx::query(&sql)
            .bind(ratchet_key)
            .execute(&self.pool.db)
            .await
        ...
        let cnt = result.rows_affected();
        if cnt > 0 {
            info!("delete {} old ratchet_key records for ({:?})", cnt, ratchet_key);
        }
     ...
     pub async fn delete_by_address_id(&self, id: u32, address: &str, room_id: u32) -> Result<()>{
        ...
        let sql = format!("delete from {} where address = ? and roomId = ? and id <= ?", self.pool.definition_ratchet_key());
        let result = sqlx::query(&sql)
            .bind(address)
            .bind(room_id)
            .bind(id)
        ...
        let cnt = result.rows_affected();
        if cnt > 0 {
            info!("delete {} old ratchet_key records for {:?}, room_id: {}, and id <= {})", cnt, address, room_id, id);
        }
        Ok(())
    }

    pub async fn get_max_id_bak(&self, address: &str, room_id: u32) -> Result<Option<u32>> {
        let sql = format!("select max(id) from {} where address = ? and roomId = ?", self.pool.definition_ratchet_key());
        let result = sqlx::query(&sql)
            .bind(address)
            .bind(room_id)
            .fetch_optional(&self.pool.db)
    ...
    pub async fn store_ratchet_key_bak(
        ...
        if max_id > 2 {
            self.delete_by_address_id(max_id - 2, address.name(), room_id).await?;
        }
        self.insert_ratchet_key(
            SignalRatchetKey {
...
impl RatchetKeyStore for KeyChatRatchetKeyStore {
    ...
    fn load_ratchet_key(&self, their_ephemeral_public: String) -> Result<String>{
        let ratchet_key = futures::executor::block_on(async move{
            let ratchet_key = self.load_ratchet_key_bak(their_ephemeral_public).await;
            ratchet_key
        });
        Ok(ratchet_key?)       
    }
    ...
    fn store_ratchet_key(
        ...
        futures::executor::block_on(async move{
            self.store_ratchet_key_bak(address, room_id, their_ephemeral_public, our_ephemeral_private).await.expect("func [store_ratchet_key] error");
        });
...
impl KeyChatSignedPreKeyStore {
    ...
    async fn get_signed_pre_key(&self, key_id: SignedPreKeyId) -> Result<SignedPreKeyRecord> {
        ...
        let sql = format!("select used, record from {} where keyId = ? order by id desc limit 1", self.pool.definition_signed_key());
        let result = sqlx::query(&sql)
    ...
    async fn save_signed_pre_key(
        ...
        let sql = format!("INSERT INTO {} (keyId, record) values (?, ?)", self.pool.definition_signed_key());
        let record_to_str = format!("{:?}", record.serialize().unwrap());
        sqlx::query(&sql)
            .bind(&key_id.to_string())
            .bind(&record_to_str)
    ...
    pub async fn remove_signed_pre_key(
        &mut self,
        key_id: SignedPreKeyId,
    ) -> Result<()> {
        let sql =format!("delete from {} where keyId = ?", self.pool.definition_signed_key());
        let result = sqlx::query(&sql)
            .bind(key_id.to_string())
            .execute(&self.pool.db)
            .await
    ...
    pub async fn generate_signed_key(&mut self, signal_identity_private_key: PrivateKey) -> Result<(u32, PublicKey, Vec<u8>)>{
        ...
        let unix_timestamp = SystemTime::now()
            ...
            .as_secs();
        let record = SignedPreKeyRecord::new(bob_sign_id.into(), unix_timestamp, &pair, &bob_signed_signature);
        self.save_signed_pre_key(bob_sign_id.into(), &record).await?;
    ...
    pub async fn del_over24h_signed_pre_key(&mut self) -> Result<()> {
        let sql = format!("delete from {} where createdAt >= datetime('now', '-1 day')", self.pool.definition_signed_key());
        let result = sqlx::query(&sql)
            .execute(&self.pool.db)
            .await
...
impl KeyChatPreKeyStore {
    ...
    async fn save_pre_key(
        ...
        let sql = format!("INSERT INTO {} (keyId, record) values (?, ?)", self.pool.definition_pre_key());
        let record_to_str = format!("{:?}", record.serialize().unwrap());
        sqlx::query(&sql)
            .bind(&key_id.to_string())
            .bind(&record_to_str)
     ...
     async fn remove_pre_key(
        ...
        let sql = format!("delete from {} where keyId = ?", self.pool.definition_pre_key());
    ...
    pub async fn del_over24h_pre_key(&mut self) -> Result<()> {
        let sql = format!("delete from {} where createdAt >= datetime('now', '-1 day')", self.pool.definition_pre_key());
        let result = sqlx::query(&sql)
            .execute(&self.pool.db)
            .await
...
impl KeyChatSignalProtocolStore {
    ...
    pub fn new(pool: LitePool, key_pair: IdentityKeyPair, registration_id: u32) -> Result<Self> {
        Ok(Self {
            ...
            identity_store: KeyChatIdentityKeyStore{pool:pool.clone(), key_pair, registration_id},
...
impl RatchetKeyStore for KeyChatSignalProtocolStore {
    ...
    fn load_ratchet_key(&self, their_ephemeral_public: String) -> Result<String> {
        self.ratchet_key_store.load_ratchet_key(their_ephemeral_public)
    }
    ...
    fn store_ratchet_key(
        ...
        self.ratchet_key_store.store_ratchet_key(address, room_id, their_ephemeral_public, our_ephemeral_private)
    }

    async fn delete_old_ratchet_key(&self, id: u32, address: String, room_id: u32) -> Result<()>{
        self.ratchet_key_store.delete_old_ratchet_key(id, address, room_id).await
    }
    ...
    async fn contains_ratchet_key(&self, their_ephemeral_public: String) -> Result<Option<bool>>{
        self.ratchet_key_store.contains_ratchet_key(their_ephemeral_public).await
    }

    async fn remove_ratchet_key(&self, their_ephemeral_public: String) -> Result<()>{
        self.ratchet_key_store.remove_ratchet_key(their_ephemeral_public).await
    }
}
...
impl SignedPreKeyStore for KeyChatSignalProtocolStore {
    ...
    async fn save_signed_pre_key(
        ...
        self.signed_pre_key_store.save_signed_pre_key(id, record).await
